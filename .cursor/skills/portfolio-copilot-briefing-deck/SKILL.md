...
- 派手な演出や過度な装飾は避ける。
- フォントは日本語の可読性を最優先にし、`Noto Sans JP`、`BIZ UDPGothic`、`Hiragino Sans`、`Yu Gothic` などの日本語フォントを使う。英字向けフォントだけで本文を組まない。
- スライドマスター、タイトル、本文、注記、図表ラベルまで日本語フォントで統一し、ウェイト差で階層を作る。
- 角丸のカードやピル形状の多用は避ける。矩形、直線、罫線、グリッドで情報を整理し、システマチックな印象を優先する。
- ボックスを使う場合も角丸は最小限にし、余白、整列、線の太さ、塗り分けで意味を表現する。装飾目的の丸みは付けない。
- 図や表は、区画をきちんと分けた長方形のパネル構成を基本とし、金融機関向けの業務資料として見やすい構造にする。
